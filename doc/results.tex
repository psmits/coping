...
The pattern exhibited by the diversity history estimated in this study (Fig. \ref{fig:diversity_est}) has some major similarities with previous mammal diversity curves \citep{Alroy2009}; most of the major increases in diversity are retained including the large diversity spike during the Miocene. The biggest difference between these two estimates is that the curve estimated here drops in value between the first and second time point while the curve from \citet{Alroy2009} increases; this can probably be attributed to the edge effect of the previously discussed partial-identifiability of \(p_{t = 1}\) \citep{Royle2008,Alroy2010c}. Because the time intervals used by \citet{Alroy2009} are slightly shorter than those used here, some of the more minor differences can most likely be attributed to that. Also, please note that the diversity values are plotted at the ``ceiling'' of each temporal interval and not at the midpoint (Fig. \ref{fig:diversity_est}).
