...
\titlegraphic{
  ...
  \includegraphics[width=2cm,height=2cm,keepaspectratio=true]{figure/iblogo3}
}
\date{}
...
\begin{frame}
  \frametitle{Structured, messy data in biology}
  \begin{center}
...
\begin{frame}
  \frametitle{Conceptualizing the question}
  % want (relative) abundance of functional groups
  % need to estimate diversity
  % what controls diversification process?
  %   functional group
  %   order
  %   mass
  %   environment (temp, plants)
  % what controls observation process?
  %   functional group
  %   size
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Multivariate regression of time/FG-varying intercept}
    \begin{align*}
      f^{\phi}_{j, t} &\sim \text{MVN}\left(
      \begin{matrix}
        U_{t, \_} \gamma^{\phi}_{j = 1} \\
        %U_{t, \_} \gamma^{\phi}_{j = 2} \\
        \vdots \\
        U_{t, \_} \gamma^{\phi}_{j = J}
      \end{matrix}, 
      \text{diag}(\tau_{f^{\phi}}) \Omega_{f^{\phi}} \text{diag}(\tau_{f^{\phi}}) \right) \\
      f^{\pi}_{j, t} &\sim \text{MVN}\left(
      \begin{matrix}
        U_{t, \_} \gamma^{\pi}_{j = 1} \\
        %U_{t, \_} \gamma^{\pi}_{j = 2} \\
        \vdots \\
        U_{t, \_} \gamma^{\pi}_{j = J}
      \end{matrix}, 
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item UC Berkeley
          \begin{itemize}
            \item Seth Finnegan, \\Adiel Klompmaker, \\Emily Orzechowski, \\Larry Taylor, \\Sara Kahanamoku, \\Josh Zimmt
          \end{itemize}
        \item UChicago
          \begin{itemize}
            \item Kenneth D. Angielczyk, \\Michael J. Foote, \\P. David Polly, \\Richard H. Ree, \\Graham Slater
            \item UChicago CEB Hinds Fund
          \end{itemize}
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
