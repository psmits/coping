...
\begin{frame}
  ...
  \begin{itemize}
    \item Random-walk priors for time-varying intercepts to control for auto-correlation.
    \item Priors predict very weak/no effects of mass on observation, origination, survival (e.g. \(\mathcal{N}(0, 0.5)\)).
    \item Priors predict very weak/no effect of group-level covariates on observation, origination, survival (e.g. \(\mathcal{N}(0, 0.5)\)).
    \item Priors favor no correlation between functional groups over time (e.g. LKJ\((2)\)).
      ...
      \begin{itemize}
        ...
        \item \textbf{Automatic Differentiation Variational Inference (ADVI)}
          \begin{itemize}
            ...
            \item approximate Bayesian inference \\assumes posterior is Gaussian
