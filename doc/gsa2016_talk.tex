...
\begin{frame}
  ...
  % 4th corner is from older ordination lit
  %   presence, traits, environment, trait X environment
  % there are two general classes of modern presence modeling
  %   species distribution models
  %   cats regression which is based on maxent theory of ecology
  % the modern fourth corner is a model that combines these two things
  %   there are multiple ways of conceiving of this
  %   the general strategy is _insert figure here_
\end{frame}
...
\begin{frame}
  \begin{block}{Concerns and conclusions}
    \begin{itemize}
      \item basic and full models have similar fits until Neogene
      \item posterior predictive simulations disimilar to observed; poor model adequacy
        \begin{itemize}
          \item previous work has never evaluated model adequacy, so no baseline for comparison
          \item second-order Markov process?
          \item try for full posterior inference
        \end{itemize}
