...
# TODO
update.taxonomy <- function(occur, key, extinct) {
  # get away from the input asap
  tt <- occur[1:100, c('order', 'family', 'genus')]
  
  # these parts are slow, use uniques and then put back in
  gen <- unique(tt[, 3])
  eol.id <- get_eolid(gen, key = key, rows = 1)  # req. internet
  eol.class <- classification(eol.id, key = key)  # req. internet

  # get the order, family information from those taxa found
  out <- llply(eol.class, function(x) {
                 if(class(x) == 'data.frame') {
                   a <- x$rank %in% c('order', 'family')
                   b <- x[a, ]
                   b[, 1]
                 } else {  # bad responses
                   NA
  ...
  # pick up here
  no.new <- apply(out, 2, is.na)
  for(ii in seq(nrow(no.new))) {
    if(any(!no.new[ii, ])) {
      tt[ii, 1:2] <- out[which(gen %in% tt[ii, 3]), ]
    }
  }

  # if the family is extinct and i know it, give it an order
  mat <- list()
  for(ii in seq(length(extinct)))
    mat[[ii]] <- tt[, 2] %in% extinct[[1]]
  


}

#update.taxonomy(occur[1:10, ], eol.key, extinct = extinct)
...
##unique(occur$order)
##unique(occur$family)
##unique(occur$family[occur$order == '' & occur$family != ''])
#
#
#
