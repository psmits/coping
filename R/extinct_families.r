...
# problem genera in families
#viverravidae <- 
miacidae <- c('Oodectes', 'Vulpavus', 'Tapocyon', 'Procynodictis', 'Uintacyon', 'Vassacyon', 'Gracilocyon')
erinaceomorpha <- c('Talpavus', 'Diacodon', 'Talpavoides', 'Diacocherus', 'Aethomylos')
homacodontidae <- c('Antiacodon')

extinct[[1]] <- list(Miacidae = miacidae,
                     Erinaceomorpha = erinaceomorpha,
                     Homacodontidae = homacodontidae)


# familes in orders
cingulata <- c('Pampatheriidae', 'Glyptodontidae')
pilosa <- c('Mylodontidae', 'Nothrotheriidae', 'Megatheriidae', 'Megalonychidae')
notoungulata <- c('Toxodontidae')
soricomorpha <- c('Plesiosoricidae', 'Proscalopidae', 'Micropternodontidae', 'Apternodontidae', 'Parpaternodontidae')
lipotyphla <- c('Geolabididae')
...
carnivora <- c('Viverravidae', 'Miacidae', 'Canidae', 'Felidae', 'Mephitidae', 'Mustelidae', 'Ursidae', 'Procyonidae')
